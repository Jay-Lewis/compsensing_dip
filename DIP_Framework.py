...
class DIP_net(object):
    ...
    def optimize_function(self, A_type, reconstruct_mode, save_flag):
        ...
        # Print / Save Progress
        if(save_flag):
            self.print_progress(out_np, total_loss, A_type)
    ...
    def print_progress(self, image, total_loss, A_type):
    ...
    def plot_loss_curves(self, losses, img_losses, A_type): #TODO: not sure if it's plotting MSE or just E
        if self.SAVE:
            full_directory = self.get_full_directory(A_type)
